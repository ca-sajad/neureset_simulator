    ...
    : QObject(parent), tableWidget(tableWidget), sessionInfoDisplay(sessionInfoDisplay), currentTableIndex(-1)
{
    ...
    int neureset_simulatorIndex = currentDir.lastIndexOf(separator + "neureset_simulator" + separator);
    if (neureset_simulatorIndex == -1)
    {
        qDebug() << "Path error";
        return;
    }

    QString basePath = currentDir.left(neureset_simulatorIndex + separator.length() + QString("neureset_simulator").length());
